...
router.get('/', async (req, res) => {
    ...
    async function initiateSession() {
        ...
        try {
            ...
            DEX-AJ-BOT.ev.on('connection.update', async (update) => {
...
└─────────────────┈ ⳹\n\n`
            ...
            if (!DEX-AJ-BOT.authState.creds.registered) {
                await delay(3000); // Wait 3 seconds before requesting pairing code
                num = num.replace(/[^\d+]/g, '');
                if (num.startsWith('+')) num = num.substring(1);

                try {
                    let code = await DEX-AJ-BOT.requestPairingCode(num);
            ...
            DEX-AJ-BOT.ev.on('creds.update', saveCreds);
        } catch (err) {
